...
function matchChoice(options, input) {
    ...
    return err(`Please reply with a number from 1 to ${options.length}.`, `Sicela uphendvule ngenombolo kusukela ku-1 kuya ku-${options.length}.`);
}
export function validateField(spec, raw, ctxOptions) {
    const input = raw.trim();
    switch (spec.type) {
        case 'text': {
            if (input.length === 0)
                return err('Please enter a value.', 'Sicela ufake umniningwane.');
            if (spec.min !== undefined && input.length < spec.min)
                return err(`Please enter at least ${spec.min} characters.`, `Sicela ufake okungenani tinhlamvu letingu-${spec.min}.`);
            if (spec.max !== undefined && input.length > spec.max)
                return err(`Please keep it under ${spec.max} characters.`, `Sicela wehlise ngephansi kwetinhlamvu letingu-${spec.max}.`);
        ...
        case 'dynamic_choice': {
            if (!ctxOptions || ctxOptions.length === 0)
                return err('No options are available right now. Please try again shortly.', 'Akukho lokungakhetfwa kwanyalo. Sicela uzame futsi ngemuva kwesikhashana.');
        ...
        case 'msisdn': {
            ...
            let normalized;
            if (digits.length === 8)
                normalized = `268${digits}`;
            else if (digits.length === 11 && digits.startsWith('268'))
                normalized = digits;
            else
                return err('Please enter a valid phone number.', 'Sicela ufake inombolo yelucingo lefanele.');
        ...
        case 'confirm': {
            ...
            if (['yes', 'y', '1', 'yebo'].includes(v))
                return { ok: true, value: 'yes' };
            if (['no', 'n', '2', 'cha'].includes(v))
